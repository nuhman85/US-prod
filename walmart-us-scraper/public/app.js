...
fileInput.addEventListener("change", () => {
  fileLabel.textContent = fileInput.files[0]?.name || "Choose a Walmart HTML file";
...
function render(products) {
  rows.replaceChildren(...products.map((product, index) => {
    ...
    const specText = Object.entries(product.specifications)
      .filter(([key]) => key !== "source_title")
      .map(([key, value]) => `${key.replaceAll("_", " ")}: ${Array.isArray(value) ? value.join(", ") : value}`)
      .join(" · ");
    row.innerHTML = `<td>${index + 1}</td><td><a class="product-link" target="_blank" rel="noreferrer"></a></td><td></td><td class="price"></td><td class="specs"></td>`;
  ...
  count.textContent = `${products.length} / 15`;
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    importButton.disabled = payload.count !== 15;
    setStatus(`Found ${payload.count} products for “${payload.searchTerm}” with item URLs and prices.`, payload.count === 15 ? "success" : "error");
